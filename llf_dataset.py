...
class TrainDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index):
        ...
        batch_images_grey = torch.zeros(self.batch_per_gpu, 1, batch_resized_height, batch_resized_width)
        ...
        for i in range(self.batch_per_gpu):
            ...
            img_grey = imread(image_path, mode='L')
            ...
            if self.random_flip == True:
                ...
                if random_flip == 1:
                    img = cv2.flip(img, 1)
                    img_grey = cv2.flip(img_grey, 1)
                    segm = cv2.flip(segm, 1)

            # note that each sample within a mini batch has different scale param
            img = imresize(img, (batch_resized_size[i, 0], batch_resized_size[i, 1]), interp='bilinear')
            img_grey = imresize(img_grey, (batch_resized_size[i, 0], batch_resized_size[i, 1]), interp='bilinear')
            ...
            img_grey = img_grey[np.newaxis, :, :]
            img_grey = torch.from_numpy(img_grey.copy()) - 116

            batch_images[i][:, :img.shape[1], :img.shape[2]] = img
            batch_images_grey[i][:, :img.shape[1], :img.shape[2]] = img_grey
        ...
        output['img_grey_data'] = batch_images_grey
        return output
...
class ValDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for this_short_size in self.imgSize:
            ...
            img_grey_resized = cv2.resize(img_grey.copy(), (target_width, target_height))
            ...
            img_grey_resized = torch.unsqueeze(img_grey_resized, 0)
            img_grey_resized_list.append(img_grey_resized)
        ...
        output['img_grey_data'] = [x.contiguous() for x in img_grey_resized_list]
