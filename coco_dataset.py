...
# Converter between COCO json id and contiguous id
def decode_segmap(temp, contiguous_category_id_to_json_id):
    segm = temp.copy()
    contiguous_ids = np.unique(temp)
    for contiguous_id in contiguous_ids:
        json_id = contiguous_category_id_to_json_id[contiguous_id]
        segm[temp == contiguous_id] = json_id
    return segm

def encode_segmap(temp, json_category_id_to_contiguous_id):
    segm = temp.copy()
    json_ids = np.unique(temp)
    for json_id in json_ids:
        contiguous_id = json_category_id_to_contiguous_id[json_id]
        segm[temp == json_id] = contiguous_id
    return segm
...
class TrainDataset(torchdata.Dataset):
    def __init__(self, opt, max_sample=-1, batch_per_gpu=1):
        ...
        # images and labels
        self.images_base = os.path.join(self.root_dataset, 'train2017')
        self.annotations_base = os.path.join(
            self.root_dataset, "annotations", 'panoptic_semseg_train2017'
        )
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            segm = encode_segmap(np.array(segm, dtype=np.uint8), self.json_category_id_to_contiguous_id)
...
class ValDataset(torchdata.Dataset):
    def __init__(self, opt, max_sample=-1, start_idx=-1, end_idx=-1):
        ...
        # images and labels
        self.images_base = os.path.join(self.root_dataset, 'val2017')
        self.annotations_base = os.path.join(
            self.root_dataset, "annotations", 'panoptic_semseg_val2017'
        )
        annotations_file = os.path.join(
